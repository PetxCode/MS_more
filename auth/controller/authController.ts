...
export const updateSingleAccount = async (req: Request, res: Response) => {
  try {
    const { accountID } = req.params;
    ...
    const accountUpdate = await prisma.crowdAuth.update({
      where: { id: accountID },
      data: {
        profile: req.body,
      },
    ...
    return res.status(201).json({
      message: "updating single Account",
      data: accountUpdate,
    });
  } catch (error) {
    return res.status(404).json({
...
export const cleanProfileAccount = async (req: Request, res: Response) => {
  try {
    const { accountID } = req.params;
